    ...
    groups[day].forEach(c => {
      ...
      card.innerHTML = `
        ...
        <div class="course-main">
          ...
          <div class="course-time">${escapeHtml(c.start)}–${escapeHtml(c.end)}${Number(c.periods||1)>1 ? ` · ${escapeHtml(c.periods)}课时连上` : ""}</div>
...
async function compressImage(file, maxSize = 1800, quality = 0.86) {
  ...
  try {
    const img = await new Promise((resolve, reject) => {
      ...
      image.onerror = () => reject(new Error("无法读取课程表图片。"));
      image.src = url;
    });
    const w = img.naturalWidth || img.width;
    const h = img.naturalHeight || img.height;
    if (!w || !h) throw new Error("课程表图片尺寸无效。");
  ...
  const rows = list.map((c,i) => `
    <div class="preview-row">
      ...
      <div>
        ...
        <small>${escapeHtml(c.day)} · ${escapeHtml(c.start)}–${escapeHtml(c.end)}${Number(c.periods||1)>1 ? ` · ${escapeHtml(c.periods)}课时连上` : ""} · ${escapeHtml(c.room || "地点未识别")}</small>
      </div>
